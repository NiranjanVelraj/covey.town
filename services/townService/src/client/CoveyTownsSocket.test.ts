...
describe('TownServiceApiSocket', () => {
  ...
  it('Invalidates the user session after disconnection', async () => {
    ...
    socket.close();
    const { socket: secondTryWithSameToken, socketDisconnected: secondSocketDisconnected } =
  ...
  it('Informs all new players when a player joins', async () => {
    ...
    const { socketConnected: connectPromise2, newPlayerJoined: newPlayerPromise2 } =
  ...
  it('Informs all players when a player disconnects', async () => {
    ...
    const { socketConnected: connectPromise2, playerDisconnected: playerDisconnectPromise2 } =
      TestUtils.createSocketClient(server, joinData2.coveySessionToken, town.coveyTownID);
    const { socket: socketWhoLeaves, socketConnected: connectPromise3 } =
  ...
  it('Informs all players when the town is destroyed', async () => {
    ...
    const { socketDisconnected: disconnectPromise2, socketConnected: connectPromise2 } =
      TestUtils.createSocketClient(server, joinData2.coveySessionToken, town.coveyTownID);
    await Promise.all([socketConnected, connectPromise2]);
