import { FriendRequests, Players, PrismaClient } from '@prisma/client';
...
export async function addFriend(player: Players, newFriendId: string): Promise<Players | null> {
  ...
  if (foundPlayer?.friendIds.includes(newFriendId)) {
    return foundPlayer;
  }
 ...
 */
export async function getSentFriendRequestsStatus(
  playerName: string,
  status: string,
 ...
 * @returns the list of friend requests or null otherwise
 */
export async function getReceivedFriendRequestsStatus(
  playerName: string,
  status: string,
): Promise<FriendRequests[]> {
  const sentFriendRequests = await prisma.friendRequests.findMany({
